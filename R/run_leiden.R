#' Leiden Clustering for a vectors of Parameters
#' @description
#' conducts Leiden clustering for multiple vectors of parameter choices.
#'
...
#' @param resolution A scalar between 0 and 1. The `resolution_parameter` from [igraph::cluster_leiden()].
#'
#' @return a named list containing classification vectors of the parameters provided.
#' @seealso [igraph::cluster_leiden()], [cccd::nng()]
#'
#' @examples
...
#'
#' run_leiden(y$pca_distance_object, c(20, 25), c(0.5, 0.8))
#'
#' @export
run_leiden <- function(data_input,
                       k_nearest_neighbors,
                       resolution){
  ...
  for(j in k_nearest_neighbors) {
    ...
    for (i in resolution) {
      ...
      h <- igraph::cluster_leiden(graph, resolution_parameter = i)
